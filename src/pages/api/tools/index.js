...
export function GET({ request }) {
  try {
    ...
    if (search) {
      const q = `%${search}%`;
      where += ' AND (name LIKE ? OR desc LIKE ? OR url LIKE ?)';
      params.push(q, q, q);
    }
    ...
    if (tag) {
      where += ' AND (tags LIKE ? OR tags LIKE ?)';
      params.push(`%"${tag}"%`, `%${tag}%`);
    }
...
export async function POST({ request }) {
  try {
    ...
    qrun('INSERT OR REPLACE INTO herramientas (id,name,url,desc,category,importance,tags,favicon,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)',
      id, body.name.trim(), body.url.trim(), (body.desc||'').trim(),
      body.category||'Otro', body.importance||'util', JSON.stringify(body.tags||[]), (body.favicon||'').trim(), now, now);
